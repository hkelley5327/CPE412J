...
// g++ heat_1d_PTH5.cpp -o heat_1d_PTH5 -lpthread
using namespace std;
#include <stdlib.h>
#include <stdio.h>
#include <iostream>
...
void *thread_total(void * arg) {
    ...
    int points_per_thread = (active_points + nthreads - 1) / nthreads;
    int start = (tid - 1) * points_per_thread + 1;
    int end = tid * points_per_thread;

    if (end > active_points) {
	end = active_points;
    }
