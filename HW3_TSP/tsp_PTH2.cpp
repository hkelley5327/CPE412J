...
void city_tours_gen(int city_order_slot, int *city_visit_order,bool *city_msk,
        int cum_tour_cost, int *thread_best_city_visit_order, int *thread_best_cost) {
   ...
   if (city_order_slot<num_cities) {
      ...
      while ((city=next_city(city,city_msk,num_cities)) >=0 ) { 
         ...
         city_tours_gen(city_order_slot+1,city_visit_order,
            city_msk, city_tour_cost, thread_best_city_visit_order, thread_best_cost);
...
void *thread_total(void * arg) {
   ...
   pthread_mutex_lock(&tour_lock);

   if (thread_best_cost < best_cost) { 
      best_cost = thread_best_cost;
      save_order(best_city_visit_order, num_cities, thread_best_city_visit_order);
   }

   pthread_mutex_unlock(&tour_lock);
...
void tour_search(void) {
   ...
   pthread_mutex_init(&tour_lock, NULL);
   ...
   pthread_mutex_destroy(&tour_lock);
}
